...
class LlavaMetaModel:
    ...
    def get_rag_score_module(self):
        module = getattr(self, 'visual_rag_score', None)
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        language_hidden_size = getattr(self.config, 'hidden_size', None)
        if language_hidden_size is None and hasattr(self.config, 'text_config'):
        ...
        if language_hidden_size is not None:
            ...
            if existing is None or \
               ...
               existing.proj.out_features != language_hidden_size:
                self.visual_rag_score = build_vision_rag_scorer(
                    vision_hidden_size=vision_tower.hidden_size,
                    language_hidden_size=language_hidden_size,
                    select_feature=mm_vision_select_feature,
                    pool_mode=rag_pool_mode,
                )
            else:
                self.visual_rag_score.select_feature = mm_vision_select_feature
                self.visual_rag_score.pool_mode = rag_pool_mode
