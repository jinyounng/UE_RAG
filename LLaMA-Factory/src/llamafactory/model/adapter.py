...
def _setup_full_tuning(
    ...
    # Always keep RAG parameters trainable even if they are under language model namespace
    RAG_PARAM_KEYWORDS = [
        "rag_projection.weight",
        "rag_projection_v.weight",
        "rag_modules.score_adapter.weight",
    ]
    for name, param in model.named_parameters():
        is_rag_param = any(key in name for key in RAG_PARAM_KEYWORDS)
        if is_rag_param:
        ...
        if not any(forbidden_module in name for forbidden_module in forbidden_modules):
            # leave requires_grad as-is for non-forbidden modules
            if cast_trainable_params_to_fp32:
                param.data = param.data.to(torch.float32)
        else:
