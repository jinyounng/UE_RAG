...
class ModelArguments(
    ...
    enable_rag: bool = field(
        default=False,
        metadata={"help": "Whether to enable internal RAG functionality."},
    )
    rag_top_k: Optional[int] = field(
        default=5,
        metadata={"help": "Number of top-k features to retrieve for RAG."},
    )
    rag_dim: Optional[int] = field(
        default=None,
        metadata={"help": "Dimension for RAG projection layers. If None, uses model's hidden size."},
    )
    rag_training_mode: Literal["rag_only", "rag_lm_joint", "auto"] = field(
        default="auto",
        metadata={
            "help": (
                "RAG training mode: "
                "'rag_only' - train only RAG modules, "
                "'rag_lm_joint' - train RAG modules and language model together, "
                "'auto' - automatically determine based on freeze_language_model setting"
            )
        },
    )
    rag_layer_selection_mode: Literal["standard", "concat_last_layer"] = field(
        default="standard",
        metadata={
            "help": (
                "RAG layer selection mode: "
                "'standard' - use standard RAG-Sequence with top-k selection, "
                "'concat_last_layer' - always include last layer + top-(k-1) from others"
            )
        },
    )
